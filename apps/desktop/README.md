...
Signed Windows packaging preserves valid vendor signatures and signs unsigned PE executables, DLLs, Python extensions and Node addons in the primary runtime before executing its smoke checks. Each new signature must match the configured certificate and carry a timestamp; invalid existing signatures, signing errors and verification errors stop the run without retries. Electron-builder preserves copied runtime executables only after checking their signature and exact equality with the prepared file, preventing repeat signing during resource copying. Checks include decimal, XML, LZMA, UUID, Pillow and lxml. Development, preparation-only and unsigned builds do not use the hardware token and can be blocked by Windows code-integrity policy; no build mode disables that policy. A passing smoke does not establish compatibility for every extension or enterprise policy.

Desktop carries independent Python and pnpm distributions, plus a Node launcher that installation points at the application's own executable in Node mode. Python includes python-docx, python-pptx, openpyxl, Pillow, lxml and XlsxWriter with their complete dependencies; it carries neither numpy nor pandas. The `load_workspace_dependencies` tool installs this payload offline on first use under `$DSH_HOME/dsh-runtimes/dsh-primary-runtime` (normally `~/.dsh/dsh-runtimes/dsh-primary-runtime`) and returns absolute interpreter, pnpm script and library paths plus `pythonDistributions`, the bundled distribution names and versions. The version report excludes user-installed additions. Office tasks prefer these libraries unless user or workspace instructions select another environment. Execute the pnpm script with the returned Node launcher. The returned Node library directory is reserved for bundled libraries, not pnpm's global installation directory.
...
The [Office conversion provider](../../packages/document/office-to-pdf/README.md) is not part of the default Desktop payload; the default Web bundle composition omits it, so `prepare:dsh` skips engine copying and its packaging check when the workspace does not declare `@deepseek-ai/libreoffice-kit`. A build that reintroduces the provider carries the target's declared native engine, or WASM when the kit declares no matching native target, and preparation then rejects a missing target engine before packaging; engine assets, licenses, and notices remain in the runtime dependency tree. Without the provider, `.doc`, `.xls`, `.ppt`, and `.pptx` previews report configuration guidance; `.docx` and `.xlsx`/`.xls` preview locally in the browser through [mammoth and SheetJS](../../packages/client/ui-sidebar-documentpreview/README.md#local-word-and-spreadsheet-preview) instead, with no engine or Host round trip.
