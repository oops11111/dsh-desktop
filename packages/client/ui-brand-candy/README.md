---
description: "Candy's own sidebar brand occupants, active unconditionally; for users and maintainers choosing or replacing brand presentation."
...
This package gives the Candy client the Candy mark and name in the sidebar, unconditionally: unlike `dsh-client-ui-brand-official`, registration is not gated by a build profile, because Candy is this deployment's only identity. The conversation hero keeps its declaring package's own fallback. Choose it for a Candy-branded deployment; a deployment with another identity should compose a different replacement brand package instead. It has no runtime state and does not affect model requests.
...
Mount this plugin in the browser roster in place of `dsh-client-ui-brand-official`, disabling that row first so the two never compete for the same single-scope slots. No build profile selection is required.
...
- [ui-brand-official](../ui-brand-official/README.md) — the row this package replaces in a Candy composition.
...
- **Placeholder artwork** — the mark and wordmark are a first-pass design pending a final asset.
