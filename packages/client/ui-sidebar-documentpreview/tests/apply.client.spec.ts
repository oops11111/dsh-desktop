...
import { SPREADSHEET_BODY_ID } from '../src/client/spreadsheet/index.ts'
...
describe('ui-sidebar-documentpreview apply', () => {
  it('registers the type, its dictionaries, and the body and title seats under the type\'s id, the body with a store and a face', async () => {
    ...
    expect(registered.map(entry => [entry.name, entry.key, entry.locale, entry.component])).toEqual([
      ...
      ['sidebar.right.tab.document', DOCX_BODY_ID, 'documentDocx', LazyDocxBody],
      ['sidebar.right.tab.document', SPREADSHEET_BODY_ID, 'documentSpreadsheet', LazySpreadsheetBody],
