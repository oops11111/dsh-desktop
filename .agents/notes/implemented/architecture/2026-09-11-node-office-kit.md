...
The [provider tests](../../../../packages/document/office-to-pdf/tests/provider.spec.ts) and [browser scenario](../../../../apps/web/tests/document-preview.e2e.ts) own DSH lifecycle, authorization, and presentation evidence; the Web bundle's default composition no longer mounts this provider (its LibreOffice engine is a Desktop payload cost the default composition avoids), so Loader composition evidence moved to the provider's own tests. Engine qualification additionally requires real DOC/DOCX/XLS/XLSX/PPT/PPTX conversion, external PDF text/font/page/image inspection, relocation and corrupted-package rejection, and same-input native/WASM performance samples. Mock helpers and microbenchmarks do not establish these outcomes. Each target's real builder and Desktop package need independent qualification; a successful local architecture does not prove the full matrix.
